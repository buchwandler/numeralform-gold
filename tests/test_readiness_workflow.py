...
from numeralform_gold.workflow import aggregate_candidates, batch_preflight


def candidate(value: str, form: str | None = None):
    return make_record(
        language="en",
        locale="en-US",
        value=value,
        ...
        source_observations=[
            {"benchmark": "fixture", "role": "gold_anchor", "source_id": f"en:{value}"}
        ],
    )
...
def test_candidate_validation_rejects_duplicates_before_batch_creation(tmp_path: Path):
    ...
    assert main(["--work-root", str(work), "batch-create", "--batch", "bad", "--candidates", str(candidates)]) == 2
    assert not (work / "batches" / "bad").exists()


def test_uniform_form_status_is_informational():
    rows = [completed(blind_review_case(candidate(str(i)), "A"), "A", "a", "family-a") for i in range(10)]
...
def test_repeated_oracle_and_decimal_echo_are_blocking():
    ...
    for i in range(10):
        ...
        row["annotation"]["oracle"] = {"canonical": "same", "accepted": ["same"], "rejected": []}
        repeated.append(row)
    repeated_report = build_review_anomaly_report(repeated)
    assert "repeated_oracle_object" in {item["code"] for item in repeated_report["blocking_signals"]}
...
def test_existing_source_observations_are_preserved(tmp_path: Path):
    ...
    write_jsonl(batch.source_observations, [{"case_id": case["case_id"], "observation": record["source_observations"][0]}])
    write_jsonl(batch.review_blind("A"), [blind_review_case(record, "A")])
    write_jsonl(batch.review_blind("B"), [blind_review_case(record, "B")])
    write_jsonl(batch.review_complete("A"), [completed(blind_review_case(record, "A"), "A", "a", "family-a")])
    write_jsonl(batch.review_complete("B"), [completed(blind_review_case(record, "B"), "B", "b", "family-b")])
...
def test_true_cli_workflow_uses_slot_artifacts_and_finalizes(tmp_path: Path):
    ...
    args = ["--work-root", str(work), "batch-create", "--batch", "pilot", "--candidates", str(candidates)]
    assert main(args) == 0
    layout = WorkLayout(work).batch("pilot")
    assert all(row["reviewer_slot"] == "B" for row in read_jsonl(layout.review_blind("B")))

    for slot, reviewer_id, family in (("A", "review-a", "family-a"), ("B", "review-b", "family-b")):
        assert main(["--work-root", str(work), "review-packet", "--batch", "pilot", "--slot", slot]) == 0
        packet = layout.review_packet(slot, 1)
        completed_rows = [completed(row, slot, reviewer_id, family) for row in read_jsonl(packet)]
        result = tmp_path / f"{slot}.result.jsonl"
        write_jsonl(result, completed_rows)
        assert main([
            "--work-root", str(work), "review-merge", "--batch", "pilot", "--slot", slot,
            "--packet-result", str(result),
        ]) == 0

    assert main(["--work-root", str(work), "review-check", "--batch", "pilot"]) == 0
    assert main(["--work-root", str(work), "adjudication-packet", "--batch", "pilot"]) == 0
    ...
    assert main([
        "--work-root", str(work), "adjudication-merge", "--batch", "pilot",
        "--packet-result", str(adj_result), "--finalize",
    ]) == 0
    corpus = tmp_path / "corpus"
    lineage = tmp_path / "lineage.jsonl"
    assert main([
        "--work-root", str(work), "batch-preflight", "--batch", "pilot", "--corpus", str(corpus)
    ]) == 0
    assert main([
        "--work-root", str(work), "batch-finalize", "--batch", "pilot", "--corpus", str(corpus),
        "--lineage", str(lineage),
    ]) == 0
    assert main([
        "--work-root", str(work), "batch-finalize", "--batch", "pilot", "--corpus", str(corpus),
        "--lineage", str(lineage), "--write",
    ]) == 0
...
def test_work_layout_lineage_is_repository_controlled(tmp_path: Path):
    assert WorkLayout(tmp_path / "external-work").lineage == Path(__file__).parents[1] / "data" / "lineage" / "review-evidence.jsonl"
...
def test_agent_bundle_resolves_role_and_batch_placeholders(tmp_path: Path):
    ...
    assert main(["--work-root", str(work), "batch-create", "--batch", "bundle", "--candidates", str(candidates)]) == 0
    output = tmp_path / "bundle.md"
    assert main([
        "--work-root", str(work), "agent-bundle", "--batch", "bundle",
        "--role", "review-b", "--out", str(output), "--reviewer-id", "review-b",
        "--model-family", "family-b",
    ]) == 0
...
def test_schema_contracts_are_json_and_slot_aware():
    completed_schema = json.loads(Path("schemas/completed-review.schema.json").read_text())
    ...
    adjudication_schema = json.loads(Path("schemas/adjudication.schema.json").read_text())
    assert adjudication_schema["properties"]["decision"]["enum"] == ["accept", "exclude", "unresolved"]
